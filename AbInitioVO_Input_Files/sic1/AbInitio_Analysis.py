...
new_directory = 'Minimum_Full_Score'
new_abin_structure_names = 'AbInitio_'
new_abin_vo_structure_name = 'AbInitioVO_'
os.makedirs(new_directory)
os.chmod(new_directory,0o777)
process_list = [abin_out_file, abin_vo_out_file]
rewrite_list = [rewrite_abin_out_file, rewrite_abin_vo_out_file]
struct_name_list = [new_abin_structure_names, new_abin_vo_structure_name]
...
for input_file_idx,input_file in enumerate(process_list):
	...
	if order_code == 'O' or order_code == 'P':
	...
	for idx,item in enumerate(new_list[0:]):
		...
		if order_code == 'P':
			for residue in test_residue_gen:
				...
				if res_num >= start_res or res_num <= end_res:
