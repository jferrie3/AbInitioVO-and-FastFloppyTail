...
new_directory = 'TESTING_NEW_ANALYSIS'
...
for input_file_idx,input_file in enumerate(process_list):
	...
	native_resi_list = []
	order_disorder_holder = np.array([1])
	order_disorder_dssp_idx_list = []
	...
	if order_code == 'O':
		...
		for idx,item in enumerate(new_list[0:]):
			test_struct = parser.get_structure('test', item['fa_name'].decode('UTF-8'))
			test_model = test_struct[0]
			dssp = DSSP(test_model, item['fa_name'].decode('UTF-8'))
			if idx == 0:
				order_disorder_holder = np.zeros([len(dssp),1])
			for resi_dssp_idx,resi_dssp in enumerate(dssp):
				if resi_dssp[2] == 'G' or resi_dssp[2] == 'H' or resi_dssp[2] == 'I' or resi_dssp[2] == 'B' or resi_dssp[2] == 'E':
					order_disorder_holder[resi_dssp_idx] += 1
		order_disorder_holder /= len(new_list)
		for idx,item in enumerate(order_disorder_holder):
			if item > 0.5:
				order_disorder_dssp_idx_list.append(idx)
				order_disorder_holder[idx] = 0.0
			else:
				order_disorder_holder[idx] = 1.0
		native_struct = parser.get_structure('native', str(new_list[0]['fa_name'].decode('UTF-8')))
		native_atoms_gen = native_struct.get_residues()
		native_atom_list = []
		for atom_idx,atom in enumerate(native_atoms_gen):
			if atom_idx in order_disorder_dssp_idx_list:
				native_atom_list.append(atom['CA'])		
	...
	for idx,item in enumerate(new_list[0:]):
		...
		if order_code == 'P':
			...
			for residue_idx,residue in enumerate(test_residue_gen):
				if residue_idx in order_disorder_dssp_idx_list:
					test_atom_list.append(residue['CA'])
	...
	disorder_dat = np.genfromtxt(sys.argv[5], dtype=[('dat1', 'f8'), ('dat2', '|S2'), ('dat3', '|S1'), ('dat4', 'f8')], delimiter=' ', skip_header=3) # 3 for raptorx

	outfd = open('DisorderMapping.diso', 'w')
	...
	outfd.write("\n")
	for line_idx,line_item in enumerate(order_disorder_holder):
		if disorder_dat[line_idx][0] < 10:
			if line_item > 0.5:
				outfd.write("   %i %s %s %.3f\n" % (disorder_dat[line_idx][0],disorder_dat[line_idx][1],'*',line_item))
			else:
				outfd.write("   %i %s %s %.3f\n" % (disorder_dat[line_idx][0],disorder_dat[line_idx][1],'.',line_item))
		elif disorder_dat[line_idx][0] < 100:
			if line_item > 0.5:
				outfd.write("  %i %s %s %.3f\n" % (disorder_dat[line_idx][0],disorder_dat[line_idx][1],'*',line_item))
			else:
				outfd.write("  %i %s %s %.3f\n" % (disorder_dat[line_idx][0],disorder_dat[line_idx][1],'.',line_item))
		elif disorder_dat[line_idx][0] < 1000:
			if line_item > 0.5:
				outfd.write(" %i %s %s %.3f\n" % (disorder_dat[line_idx][0],disorder_dat[line_idx][1],'*',line_item))
			else:
				outfd.write(" %i %s %s %.3f\n" % (disorder_dat[line_idx][0],disorder_dat[line_idx][1],'.',line_item))
	outfd.close()
	#np.savetxt('Disorder_Mapping.txt', order_disorder_holder, fmt='%s', delimiter=' ', newline='\n')
	os.chdir('..')
